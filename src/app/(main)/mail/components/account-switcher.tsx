...
import { User, LogOut, Settings, Plus, Mail, Check } from "lucide-react"
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        ...
        >
          <Avatar className="h-7 w-7">
            ...
            <AvatarFallback>{currentAccount ? getInitials(currentAccount.label) : "ML"}</AvatarFallback>
          </Avatar>
          
          {!isCollapsed && (
            <div className="flex flex-col items-start leading-none">
              <span className="font-semibold text-sm">{currentAccount?.label}</span>
      ...
      <DropdownMenuContent align="start" className="w-56">
        ...
        {/* 账户切换组 */}
        <DropdownMenuRadioGroup value={selectedAccount} onValueChange={handleAccountChange}>
          {accounts.map((account) => (
            <DropdownMenuRadioItem key={account.email} value={account.email} className="py-2">
        ...
        <DropdownMenuItem asChild>
          <Link href="/auth/logout" className="flex items-center text-red-500 hover:text-red-500 focus:text-red-500">
