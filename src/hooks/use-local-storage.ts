...
/**
 * 高级本地设置管理Hook - 用于复杂的用户设置和偏好
 * 整合了用户界面、仪表盘、视图设置、最近项目、草稿和搜索历史
 */
export function useLocalSettings({ userId }: { userId: string }) {
  ...
  // 最近访问的项目状态
  const [recentItems, setRecentItems] = React.useState<RecentlyViewed[]>([]);
