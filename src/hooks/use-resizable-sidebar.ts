    ...
    (collapsed: boolean) => {
      ...
      setTimeout(() => {
        if (collapsed) {
          ...
          // 折叠时，重新分配空间比例
          // 1. 获取侧边栏原本的大小
          const originalNavSize =
            expandedSizesRef.current[0] || defaultLayout[0];
          // 2. 设置侧边栏为最小尺寸
          ...
          if (
            sizes.length >= 3 ||
            expandedSizesRef.current.length >= 3 ||
            defaultLayout.length >= 3
          ) {
            // 计算第二和第三个面板的原始大小
            const panel2Size =
              expandedSizesRef.current[1] || sizes[1] || defaultLayout[1];
            const panel3Size =
              expandedSizesRef.current[2] || sizes[2] || defaultLayout[2];
            ...
            const panel2Size =
              sizes[1] || expandedSizesRef.current[1] || defaultLayout[1];
          ...
          if (expandedLayout[0] < 15) {
            ...
            if (expandedLayout.length >= 3) {
              ...
              expandedLayout[1] = Math.floor(
                (100 - 16) * (expandedLayout[1] / totalOther)
              );
